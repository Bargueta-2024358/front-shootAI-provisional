// src/render.js
// Dibuja el overlay futurista sobre un canvas: foto + esqueleto + insights + HUD.
...
const THEME = {
  ...
  nodeCore: "#38f0d0",
  warm: "#ffb347",
  symmetry: "rgba(110,168,255,0.9)",
  labelBg: "rgba(7,9,13,0.82)",
  labelBorder: "rgba(56,240,208,0.6)",
  text: "#e7edf3",
  accent: "#38f0d0",
};

// Layers por defecto (se sobreescriben desde app.js con los toggles).
...
function drawLeader(ctx, x1, y1, x2, y2, S) {
  ctx.save();
  ctx.strokeStyle = "rgba(231,237,243,0.5)";
  ctx.lineWidth = 1 * S;
  ctx.beginPath();
  ctx.moveTo(x1, y1);
  ctx.lineTo(x2, y2);
...
function drawWatermark(ctx, W, H, S) {
  ...
  ctx.fillStyle = "rgba(56,240,208,0.7)";
  ctx.textBaseline = "top";
  ctx.fillText("ATELIER SCAN", 18 * S, 16 * S);
